...
OPENING_HOURS = (('09:30', '23:00'), ('09:30', '23:00'), ('08:30', '23:00'),
                 ('12:30', '23:00'), ('09:30', '23:00'), ('10:00', '22:00'),
                 ('10:00', '21:30'))
...
def calc_interval(time_point: time) -> int:
    ...
    first_slot = timedelta(hours=PLOT_DURATION[0].hour,
                           minutes=PLOT_DURATION[0].minute)
    delta = timedelta(hours=time_point.hour,
                      minutes=time_point.minute) - first_slot
...
def generate_slots() -> Tuple[List[Tuple[time, time]], List[str]]:
    ...
    while slot < PLOT_DURATION[1]:
        ...
        y_labels.append(
            f'{slot.isoformat("minutes")} - {next_slot.isoformat("minutes")}')
        slot = next_slot

    return slots, y_labels


def prepare_heatmap(data_path: str,
                    intervals: List[Tuple[time, time]]) -> List[List[float]]:
    ...
    avgs = [[RunningAverage() for _ in range(len(WEEKDAYS))]
...
def plot(data_path: str, outfile: Optional[str]):
    ...
    cmap = matplotlib.colors.ListedColormap([
        'darkred', 'red', 'darkorange', 'orange', 'yellow', 'lightgreen',
        'green'
    ])
